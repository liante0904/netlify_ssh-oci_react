...
> 문서 기준 시점: **2026-08-24**. 현재 UI/API 구현과 대조한 로컬 소스 기준 문서다.
...
- **실시간 리포트**: 29개 증권사의 리포트를 실시간으로 수집하여 제공합니다. (2026-07-01: 성능 향상을 위한 백엔드 ORM -> Raw SQL 전환 및 전용 `/recent` API 호출을 통한 대시보드 로드 부하 최적화 반영)
...
- `npm run test`: 단위·원격 API 통합·관리자 상태 검증을 순서대로 실행합니다. 원격 데이터 품질 조건은 코드 변경과 별도로 실패할 수 있습니다.
