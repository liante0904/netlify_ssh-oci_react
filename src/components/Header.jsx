...
const Header = forwardRef(({ isNavVisible, menuOpen }, ref) => {
  ...
  return <HeaderLayout refProp={ref} hidden={!isNavVisible} layoutMenuOpen={menuOpen} onHome={() => handleButtonClick('home')} badge={renderTelegramBadge} onSearch={() => handleButtonClick('search')} user={telegramUser} onNotifications={handleNotificationClick} active={activePopover} unread={unreadCount} menuOpen={isTopMenuOpen} toggleMenu={toggleMenuTop} closePopover={closePopover} keywordState={keywordState} authLoading={isAuthenticating} login={loginWithTelegram} notifications={visibleNotifications} loading={isLoadingNotifications} readIds={readNotifyIds} markAllAsRead={() => markAllAsRead(visibleNotifications.flatMap((item) => [getNotificationKey(item), item.id]))} onNotification={handleNotificationItemClick} toast={notificationToast} setActive={setActivePopover} logout={logout} selectedCompany={selectedCompanyOrder} companyChange={handleCompanyChange} headerClick={handleButtonClick} boards={boards} selectedBoard={activeSearch.board} boardChange={handleBoardChange} onOpenSettings={handleOpenSettings} theme={theme} themePreference={themePreference} onToggleTheme={toggleTheme} />;
});
