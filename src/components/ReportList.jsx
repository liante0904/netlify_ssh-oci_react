...
export default function ReportList({ onWriterClick }) {
  ...
  useEffect(() => { if (!favoriteItems.length) return; setFavorites((current) => { const next = { ...current }; favoriteItems.forEach((item) => { next[item.report_id] = true; }); localStorage.setItem('report_favorites', JSON.stringify(next)); return next; }); }, [favoriteItems]);
  ...
  useEffect(() => { setCollapsedDates({}); setCollapsedFirms({}); setExpandedSummaries({}); setSummaryRequestedIds(new Set()); window.scrollTo(0, 0); }, [location.pathname, searchQuery, sortBy]);
